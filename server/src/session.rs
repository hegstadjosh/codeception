...
pub struct Session {
    ...
    /// true if running in tmux (recon can detect status accurately, switch to it, send messages).
    /// false if running in a regular terminal (limited control — no pane capture, no focus/switch).
    pub managed: bool,
}
...
pub fn discover_sessions(prev_sessions: &HashMap<String, Session>) -> Vec<Session> {
    ...
    for entry in entries.flatten() {
        ...
        for jentry in jsonl_files.flatten() {
            ...
            if matched_session_ids.contains(&session_id) {
                if let Some(existing) = sessions.iter_mut().find(|s| s.session_id == session_id) {
                    ...
                    if new_size > existing_size {
                        ...
                        existing.messages = crate::conversation::read_conversation_messages(&path, 50);
            ...
            let messages = crate::conversation::read_conversation_messages(&path, 50);
    ...
    // --- Second pass: discover non-tmux sessions from ~/.claude/sessions/*.json ---
    // This catches Claude Code instances running in regular Terminal windows.
    let pid_session_map = read_pid_session_map();
    for (&pid, file_info) in &pid_session_map {
        // Skip sessions already found via tmux
        if matched_session_ids.contains(&file_info.session_id) {
            continue;
        }
        // Also skip PIDs already claimed by tmux sessions
        let known_pids: std::collections::HashSet<i32> = sessions
            .iter()
            .filter_map(|s| s.pid)
            .collect();
        ...
        let jsonl_path = find_jsonl_by_session_id(&file_info.session_id);

        if let Some(path) = jsonl_path {
            let prev = prev_sessions.get(&file_info.session_id);
            let info = parse_jsonl(
                &path,
                prev.map(|s| s.last_file_size).unwrap_or(0),
                prev.map(|s| s.total_input_tokens).unwrap_or(0),
            ...
            let status = if pid_alive {
                determine_status_from_mtime(&path)
            } else {
                SessionStatus::Idle
            };

            let messages = crate::conversation::read_conversation_messages(&path, 50);
            matched_session_ids.insert(file_info.session_id.clone());
            sessions.push(Session {
            ...
            // PID alive but no JSONL yet — brand-new non-tmux session
            matched_session_ids.insert(file_info.session_id.clone());
            sessions.push(Session {
                session_id: file_info.session_id.clone(),
        ...
        // If PID is dead and no JSONL, skip entirely — nothing to show
    }
...
/// Determine session status for non-tmux sessions using JSONL file mtime.
/// Without tmux we can't read the TUI, so we use a simple heuristic:
///   - Modified in last 30s → Working (actively generating)
///   - Older → Idle
/// We cannot distinguish "Input" (waiting for user) without tmux — that's the trade-off.
fn determine_status_from_mtime(path: &Path) -> SessionStatus {
