...
use crate::summarizer::SessionSummary;
use crate::tmux;
...
pub struct App {
    ...
    pub conversation_session_idx: Option<usize>,
    pub conversation_scroll: usize,
    pub conversation_hide_tools: bool,
    summaries: HashMap<String, SessionSummary>,
    last_summarize_tick: u64,
    summarize_interval: u64,
...
impl App {
    pub fn new() -> Self {
        App {
            ...
            conversation_session_idx: None,
            conversation_scroll: 0,
            conversation_hide_tools: true,
            summaries: HashMap::new(),
            last_summarize_tick: 0,
            summarize_interval: 15, // 30 seconds at 2s tick rate
            prev_sessions: HashMap::new(),
            prev_statuses: HashMap::new(),
        }
    }

    /// Refresh sessions. Returns `true` if any session transitioned to Input.
    pub fn refresh(&mut self) -> bool {
        let old_statuses = std::mem::take(&mut self.prev_statuses);
        ...
        // Check for transitions TO Input
        let mut bell = false;
        for s in &sessions {
            if s.status == SessionStatus::Input {
                match old_statuses.get(&s.session_id) {
                    Some(SessionStatus::Input) => {} // already was Input, no bell
                    Some(_) => bell = true,          // transitioned to Input
                    None => {}                       // new session, no bell
                }
            }
        }
        ...
        // Rolling summarization (throttled).
        if self.summarize_enabled
            && self.tick.wrapping_sub(self.last_summarize_tick) >= self.summarize_interval
        {
            self.last_summarize_tick = self.tick;
            for session in &self.sessions {
                let prev = self.summaries.get(&session.session_id);
                let tc = crate::summarizer::total_chars(&session.messages);
                if let Some(updated) = crate::summarizer::summarize_session(
                    &prev.cloned(),
                    &session.messages,
                    &session.messages,
                    ...
                    self.summaries
                        .insert(session.session_id.clone(), updated);
                }
            }
        }
    ...
    pub fn filtered_indices(&self) -> Vec<usize> {
        ...
        self.sessions
            ...
            .enumerate()
            .filter(|(_, s)| {
                let session_label = s.tmux_session
                    .as_deref()
                    .map(|n| n.to_string())
                    .unwrap_or_else(|| {
                        format!("term:{}", s.pid.map(|p| p.to_string()).unwrap_or_default())
                    });
                s.project_name.to_lowercase().contains(&query)
                    || session_label.to_lowercase().contains(&query)
            })
            .map(|(i, _)| i)
            .collect()
    }
    ...
    pub fn handle_key(&mut self, key: KeyEvent) {
        ...
        match self.view_mode {
            ...
            ViewMode::Conversation => self.handle_key_conversation(key),
        }
    }
        ...
        match key.code {
            ...
            KeyCode::Char('c') => {
                if let Some(real_idx) = self.resolve_selected() {
                    self.conversation_session_idx = Some(real_idx);
    ...
    fn handle_key_conversation(&mut self, key: KeyEvent) {
        match key.code {
            KeyCode::Char('q') | KeyCode::Esc => {
                self.view_mode = ViewMode::Table;
                self.conversation_session_idx = None;
            }
            KeyCode::Char('j') | KeyCode::Down => {
            ...
            KeyCode::Char('G') => {
                // Jump to bottom — set a large value, the renderer will clamp
                self.conversation_scroll = usize::MAX;
            }
            KeyCode::Char('t') => {
                self.conversation_hide_tools = !self.conversation_hide_tools;
                self.conversation_scroll = 0;
            }
            KeyCode::Char('r') => {
                // Re-read conversation messages for this session
                if let Some(idx) = self.conversation_session_idx {
                    if let Some(session) = self.sessions.get_mut(idx) {
                        session.messages = crate::conversation::read_conversation_messages(
                            &session.jsonl_path,
                            500,
                        );
                    }
    ...
    pub fn summary_for(&self, session_id: &str) -> Option<&SessionSummary> {
